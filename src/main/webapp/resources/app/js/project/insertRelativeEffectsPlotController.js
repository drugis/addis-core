...
define(['lodash'], function(_) {
  ...
  var InsertRelativeEffectsTableController = function($scope, $stateParams, $modalInstance, $q, AnalysisResource,
    ...
    function selectedAnalysisChanged() {
      ...
      if ($scope.selections.analysis.interventionInclusions) {
        $scope.selections.baselineTreatment = $scope.selections.analysis.interventionInclusions[0];
      }
    ...
    function insertRelativeEffectsPlot() {
      if ($scope.selections.regressionLevel) {
        callback(ReportDirectiveService.getDirectiveBuilder('relative-effects-plot')($scope.selections.analysis.id,
          $scope.selections.model.id, $scope.selections.baselineTreatment.interventionId, $scope.selections.regressionLevel));
      } else {
        callback(ReportDirectiveService.getDirectiveBuilder('relative-effects-plot')($scope.selections.analysis.id,
          $scope.selections.model.id, $scope.selections.baselineTreatment.interventionId));
      }
      $modalInstance.close();
    }
  };
