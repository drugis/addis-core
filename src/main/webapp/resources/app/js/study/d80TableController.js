  ...
  var D80TableController = function (
    ...
    $q.all(allThePromises).then(function () {
      ...
      $scope.selected = {
        ...
        measurementMoment: D80TableService.getInitialMeasurementMoment(
    ...
    function buildTable() {
      $q.all($scope.resultsPromises).then(function (results) {
        $scope.measurements = D80TableService.buildMeasurements(
          results,
          $scope.selected.measurementMoment.uri,
          $scope.endpoints
        );
        buildEstimateRows();
      });
    ...
    function createArms() {
      return _.map($scope.arms, function (arm) {
        if ($scope.primaryEpoch) {
          arm.activity = D80TableService.getActivityForArm(
            arm,
            $scope.activities,
            $scope.designCoordinates,
            $scope.primaryEpoch
          );
          arm.treatmentLabel =
            !arm.activity || arm.activity.treatments.length === 0
              ? '<treatment>'
              : D80TableService.buildArmTreatmentsLabel(
                  arm.activity.treatments
                );
        }
        return arm;
      });
    ...
    function buildEstimateRows() {
      EstimatesResource.getEstimates({
        ...
        $scope.effectEstimateRows = D80TableService.buildEstimateRows(
          estimateResults,
          $scope.endpoints,
          $scope.arms
        );
