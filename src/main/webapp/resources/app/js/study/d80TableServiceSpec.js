  ...
  describe('the d80tableservice', function () {
    ...
    describe('buildResultsByEndpointAndArm', function () {
      it('should build a map of endpoints to arms to results for the data fo the primary measurement moment', function () {
        ...
        var result = d80tableservice.buildResultsByEndpointAndArm(
          results,
          'primaryMomentUri'
        );
    ...
    describe('buildMeasurements', function () {
      it('should build the measurements from the results primary measurement moment uri and endpoints', function () {
        ...
        var result = d80tableservice.buildMeasurements(
          results,
          'primaryMomentUri',
          endpoints
        );
      ...
      it('should build build an array of rows with the label value pairs of the estimates', function () {
        ...
        var expectedResult = [
          ...
          },
          {
            rowLabel: 'Risk ratio',
            rowValues: [
              estimateResults.estimates.endpoint1Uri[0].pointEstimate.toFixed(
                2
              ),
              '<point estimate>'
            ]
          },
          {
            ...
            rowValues: [
              '(' +
                estimateResults.estimates.endpoint1Uri[0].confidenceIntervalLowerBound.toFixed(
                  2
                ) +
                ', ' +
                estimateResults.estimates.endpoint1Uri[0].confidenceIntervalUpperBound.toFixed(
                  2
                ) +
                ')',
              '<confidence interval>'
          ...
          },
          {
            rowLabel: 'P-value',
            rowValues: [
              estimateResults.estimates.endpoint1Uri[0].pValue.toFixed(2),
              '<P-value>'
            ]
          }
        ];
        var result = d80tableservice.buildEstimateRows(
          estimateResults,
          endpoints,
          arms
        );
    ...
    describe('buildResultsObject', function () {
      it('should return the appropriate resultsObject', function () {
        ...
        expect(
          d80tableservice.buildResultsObject(armResults, endpoint, armUri)
    ...
    describe('buildArmTreatmentsLabel', function () {
      it('should return the appropriate treatment label for the arm', function () {
        ...
        var expectedResult =
          'Floepsetine 10 mg per 1 day(s) + Plopsatine 5-10 ml per 12 hour(s)';
        expect(d80tableservice.buildArmTreatmentsLabel(treatments)).toEqual(
          expectedResult
        );
        expect(function () {
    ...
    describe('getInitialMeasurementMoment', function () {
      it('should return the first measurement moment in the list if there is no primary epoch', function () {
        ...
        var primaryEpoch;
        var result = d80tableservice.getInitialMeasurementMoment(
          measurementMoments,
          primaryEpoch
        );
    ...
    describe('getActivityForArm', function () {
      it('should return the activity that is on the arm for the primary epoch', function () {
        ...
        var result = d80tableservice.getActivityForArm(
          arm,
          activities,
          designCoordinates,
          primaryEpoch
