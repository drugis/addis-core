...
/**
 * Created by daan on 8-7-16.
 */
public class NiceJsonLDWriter extends WriterDatasetRIOTBase {

  private final RDFFormat format ;

  public NiceJsonLDWriter(RDFFormat syntaxForm) {
    format = syntaxForm ;
  }
  ...
  private void serialize(Writer writer, DatasetGraph dataset, PrefixMap prefixMap, String baseURI) {
    ...
    try {
      ...
      opts.setEmbed(false);
      opts.outputForm = JsonLdConsts.EXPANDED;
      ...
      obj = JsonLdProcessor.expand(obj, opts);
  ...
  private static void addPrefixes(Map<String, Object> ctx, PrefixMap prefixMap) {
    ...
    for ( Map.Entry<String, IRI> e : pmap.entrySet() ) {
      String key = e.getKey() ;
      if ( key.isEmpty() )
