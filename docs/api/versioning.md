...
Use is also made of Dublin Core Terms, abbreviated as `dct:`.
...
Note that skolemization must happen before changesets are calculated, and that if clients create new blank nodes, they must retrieve the updated graph to learn the skolemized ID of those blank nodes.
...
If the header is not specified, the action is executed against the latest version.
 ...
 2. All write requests can specify the `X-EventSource-Author` header to set the author attribute of the new version, and `X-EventSource-Title` and `X-EventSource-Description` to provide a short and long description respectively. The `X-EventSource-Description` header uses [Base 64 Encoding](http://tools.ietf.org/html/rfc4648) to preserve formatting (TODO: or use different string encoding?).
...
Host: example.com
X-EventSource-Version: :version
X-EventSource-Author: :creatorUri
X-EventSource-Title: :messageTitle
```
...
dataset:ubb245f8sz a es:Dataset ;
  dct:date "2014-09-24T12:40:03+0000"^^xsd:dateTime ;
  dct:creator <http://example.com/GreenGoblin> ;
  es:head version:3ucq3j5c7u .

version:3ucq3j5c7u a es:DatasetVersion ;
  dct:date "2014-09-24T12:40:03+0000"^^xsd:dateTime ;
  dct:creator <http://example.com/GreenGoblin> ;
  dct:title "Initial version" .
```
...
X-EventSource-Author: http://example.com/GreenGoblin
X-EventSource-Title: Peter Parker is Spiderman
...
X-EventSource-Author: http://example.com/PeterParker
X-EventSource-Title: The Green Goblin is a liar!
...
dataset:qmk2x16nz1 a es:Dataset ;
  dct:date "2014-09-24T12:49:18+0000"^^xsd:dateTime ;
  dct:creator <http://example.com/Spiderman> ;
...
version:g05ri5qvvq a es:DatasetVersion ;
  dct:date "2014-09-24T12:58:16,835290832+0000"^^xsd:dateTime ;
  dct:creator <http://example.com/Spiderman> ;
  dct:title "The Green Goblin is a liar!" ;
...
version:7wi4xglx1c a es:DatasetVersion ;
  dct:date "2014-09-24T12:45:25,048366032+0000"^^xsd:dateTime ;
  dct:creator <http://example.com/GreenGoblin> ;
  dct:title "Peter Parker is Spiderman" ;
  dct:description "It is time the world knew...\nThat Peter Parker is Spiderman!" ;
